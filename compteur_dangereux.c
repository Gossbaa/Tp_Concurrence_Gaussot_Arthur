...
void* incrementeur_thread(void* arg) {
    for (int i = 0; i < INCREMENT; i++) {
        // Il y a une Race Condition ici car la modification du compteur peut etre effectuée plusieurs fois en même temps étant donné que la variable est partagée. 
        compteur_global++;  
    }
    return NULL;
}
